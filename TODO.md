...
- [x] Add additive tracker confidence / failure warnings: extend `quality.PathQuality` with `warnings`, `tracking_coverage_ratio`, and `tracking_lost_frames` (all optional, backward-compatible), populate them from `barbell_tracker` via the new `quality.analyze_tracking_warnings` / `attach_tracking_warnings` helpers, validate the new fields in `analysis_schema`, surface them in the HTML report and mobile prototype (UI keeps working when the fields are absent), and cover the behaviour with focused tests in `tests/test_quality.py` and `tests/test_analysis_schema.py`. Compact fixture metrics are unchanged; golden regression tests still pass.
- [x] Add a local session history prototype using `localStorage`: save up to five recent analysis payloads with compact metadata after JSON/demo/service analysis, restore summaries without touching the current video, and support delete/clear controls while handling unavailable or corrupt storage gracefully.
- [x] Add lift-specific analysis presets in the mobile prototype: squat, bench press, deadlift, and overhead press presets fill ROI/tracker/scale and rep segmentation options, expose those options for manual tuning, and send them through the local analysis service form.
